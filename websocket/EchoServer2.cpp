    ...
    uWS::App({
        ...
        .message = [](auto *ws, std::string_view message, uWS::OpCode opCode) {
            ...
            for(int i=0; i<1024*1000; i++) {
                auto sendStatus = ws->send("hello world message test hello world message test hello world message testhello world message testhello world message testhello world message test", opCode);
                if (sendStatus == uWS::WebSocket<false, true,PerSocketData>::SendStatus::BACKPRESSURE
                   || sendStatus == uWS::WebSocket<false, true,PerSocketData>::SendStatus::DROPPED) {
                    printf("hello world sendStatus %d %d\n", i, sendStatus);   
                    break;
                }
            }
        },
