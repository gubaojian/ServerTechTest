...
void test_normal() {
    ...
    SSL *client_ssl = NULL;
    if (BIO_get_ssl(server_ssl_bio, &client_ssl) <= 0) {
        handle_ssl_error("SSL BIO_get_ssl");
    }
...
void test_part_read() {
    ...
    SSL *client_ssl = NULL;
    if (BIO_get_ssl(client_ssl_bio, &client_ssl) <= 0) {
        handle_ssl_error("SSL BIO_get_ssl");
    }
    ...
    // 8. 客户端接收数据
    char decrypted[BUF_SIZE] = {0};
    ret = BIO_read(client_ssl_bio, decrypted, 10);
    if (ret <= 0) handle_ssl_error("Client read failed");
    {
        BIO_write(server_ssl_bio, PLAINTEXT, strlen(PLAINTEXT));
        int readSize = strlen(PLAINTEXT);
        printf("Server sent %d bytes: %s\n", readSize, PLAINTEXT);
        int maxRetryRead = 4;
        int retry = 0;
        int totalRead = 0;
        while (retry < maxRetryRead) { //may have remain data, so should read util none data return
            int ret2 = BIO_read(client_ssl_bio, decrypted + ret + totalRead, readSize);
            bool canReadNext = true;
            if (ret2 <= 0) {
                canReadNext = false;
                int error = SSL_get_error(client_ssl, ret2);
                ...
                printf("Client read failed %d  %d %d error %d canIgnoreError %d\n",
                    BIO_should_retry(client_ssl_bio),
                    BIO_should_read(client_ssl_bio),
                    BIO_should_write(client_ssl_bio),
                    error, canIgnoreError);
            }
            if (ret2 >= 0) {
                totalRead += ret2;
            }
        ...
        printf("Client part read %d bytes: %s\n", ret + totalRead, decrypted);
    }
