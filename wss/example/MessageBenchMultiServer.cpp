...
std::vector<uWS::WebSocket<true, true, PerSocketData>*> admins;
    ...
    uWS::SSLApp({
        ...
        .message = [](auto *ws, std::string_view message, uWS::OpCode opCode) {
            if (message.starts_with("start_") == true) {
                ...
                std::cout << "start new turn " << " admin size " << admins.size() << std::endl;
            }
             messageNum++;
            if (messageEnd) {
            ...
            if (message.starts_with("end_") == true) {
                 ...
                 std::cout << "start new turn total message " << messageNum << " admin size " << admins.size() << std::endl;
            ...
            if (data->adminId < 0) {
                data->adminId  = roundRobin%admins.size();
            }
            /* We simply echo whatever data we get */
            int server = data->adminId; //就近绑定机器转发。这样方便定位问题， 也减少socket切换开销。
        ...
        .drain = [](auto *ws) {
            std::cout << "message drain " << std::endl;
             auto adminWs = ws;
