...
int main() {
    ...
    uWS::WebSocket<false, true, PerSocketData>*  adminWs;
    ...
    uWS::App({
        ...
        .message = [&adminWs](auto *ws, std::string_view message, uWS::OpCode opCode) {
            ...
            if (adminWs != ws) {
                if (pressure) {
                    ...
                    if (status ==  uWS::WebSocket<false, true,PerSocketData>::SendStatus::BACKPRESSURE) {
                        std::cout << "backpressure" << std::endl;
                        pressure = true;
                    } else if (status ==  uWS::WebSocket<false, true,PerSocketData>::SendStatus::DROPPED) {
        ...
        .drain = [&adminWs](auto *ws) {
            ...
            while(adminBuffer->size() > 0 ) {
                ...
                if (status ==  uWS::WebSocket<false, true,PerSocketData>::SendStatus::DROPPED) {
