...
/**
 ...
 1024kb数据：
 yyjson zstd tojson used 20758ms
 yyjson tojson speed 2313.61mb
 yyjson tojson compress 1678.47mb
 yyjson tojson json len 1213
 yyjson tojson compress json len 880
 

 level 3
 
 yyjson zstd tojson used 21957ms
 yyjson tojson speed 2313.61mb
 yyjson tojson compress 1678.47mb
 yyjson tojson json len 1213
 yyjson tojson compress json len 880
 ...
 yyjson snappy tojson used 4002ms
 yyjson tojson speed 2313.61mb
 yyjson tojson compress 2319.34mb
 yyjson snappy len 1216
...
int event_bus_json_zstd_test_main(int argc, const char * argv[]) {
    ...
    for(int i=0; i<length; i++){
        ...
        jsonLength = len;
       size_t compressLen = ZSTD_compress(zstdBuffer, 2048, json, len, 1);
    ...
    start = std::chrono::high_resolution_clock::now();
    char  snappyBuffer[2048];
    int64_t snappytotalCompressLength = 0;
    ...
    for(int i=0; i<length; i++){
        ...
        jsonLength = len;
        snappy::RawCompress(json, len, snappyBuffer, &snappylen);
        if (!snappy::IsValidCompressedBuffer(snappyBuffer, snappylen)) {
            std::cout << "snappy error" << std::endl;
            break;
        }
        snappytotalCompressLength += snappylen;
        if (json) {
            free((void *)json);
        }
        // Free the doc
    ...
    used = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    std::cout << "yyjson snappy tojson used " << used.count() << "ms" << std::endl;
    std::cout << "yyjson tojson speed " << (length*jsonLength)/(1024*1024.0) << "mb" << std::endl;
    std::cout << "yyjson tojson compress " << (snappytotalCompressLength)/(1024*1024.0) << "mb" << std::endl;
    
    std::cout << "yyjson snappy len " << snappylen << std::endl;
