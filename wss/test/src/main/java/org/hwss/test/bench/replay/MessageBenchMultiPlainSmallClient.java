 ...
 * 时间可能大部分消耗在发送端。
 *
 * start new turn total message 7999960
 * receive used 6858.5ms
 *
 * 4个线程 117万每秒
 *
 * received message size : 2000002
 * received message used : 6859
 *
 * */
public class MessageBenchMultiPlainSmallClient extends WebSocketClient {

    private List<String> messages = new ArrayList<>();
    private  long startTime = 0;


    public MessageBenchMultiPlainSmallClient(URI serverUri) {
        super(serverUri);
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        WebSocketClient mainClient = new MessageBenchMultiPlainSmallClient(new URI("ws://localhost:9002/wss?app_id=434&group=client"));
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                // Create all-trusting host name verifier
                 try {
                     WebSocketClient client = new MessageBenchMultiPlainSmallClient(new URI("ws://localhost:9002/wss?app_id=434&group=client"));
