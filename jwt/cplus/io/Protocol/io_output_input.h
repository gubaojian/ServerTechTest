...
namespace wsg {
    namespace gateway {
        ...
        public:
            ...
            void writeVarInt(size_t value) {
                ...
                if (value <= 0x7F) {
                    ...
                    uint8_buffer[0] = ((value & 0x7F) | 0x80);
                    uint8_buffer[1] = (value >> 7);
                    position+=2;
                } else if (value <= 0x1FFFFF) {
                    uint8_buffer[0] = ((value & 0x7F) | 0x80);
                    uint8_buffer[1] = (((value >> 7) & 0x7F) | 0x80);
                    uint8_buffer[2] = (value >> 14);
                    position+=3;
                } else if (value <= 0xFFFFFFF) {
                    uint8_buffer[0] = ((value & 0x7F) | 0x80);
                    uint8_buffer[1] = (((value >> 7) & 0x7F) | 0x80);
                    uint8_buffer[2] = (((value >> 14) & 0x7F) | 0x80);
                    uint8_buffer[3] = (value >> 21);
                    position+=4;
                } else {
                    uint8_buffer[0] = ((value & 0x7F) | 0x80);
                    uint8_buffer[1] = (((value >> 7) & 0x7F) | 0x80);
                    uint8_buffer[2] = (((value >> 14) & 0x7F) | 0x80);
                    uint8_buffer[3] = (((value >> 21) & 0x7F) | 0x80);
                    uint8_buffer[4] = (value >> 28);
                    position+=5;
                }
            }
        ...
        public:
            ...
            int32_t readVarInt() {
                auto bts = (const int8_t*)(buffer);
                if (bts[position] >= 0) { //0x7F， none need uint8_t for negative byte 0-255 case
                    return (bts[position++]);
                }
                if ((position + 1) < bufferSize && bts[position + 1] >= 0) {
                    // avoid multiple unsequenced modifications
                    int32_t value =  ((bts[position]) & 0x7F) |
                           ((bts[position + 1]) << 7);
                    position += 2;
                    return value;
                }
                ...
                if ((position + 3) < bufferSize && bts[position + 3] >= 0) {
                    int32_t value = ((bts[position]) & 0x7F) |
                           ((bts[position + 1]) & 0x7F) << 7 |
                           ((bts[position + 2]) & 0x7F) << 14 |
                           ((bts[position + 3]) << 21);
                    position += 4;
                    return value;
                }
                if ((position + 4) < bufferSize && bts[position + 4] >= 0) {
                    if (((bts[position + 4]) & 0x7F) > 0x0F) {
                        setErrorFormatFlag();
                        return 0;
                    }
                     int32_t value = ((bts[position]) & 0x7F) |
